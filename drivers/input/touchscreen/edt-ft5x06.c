				   ...
				   u16 rd_len, u8 *rd_buf)
{
    ...
    printk(KERN_INFO "EDT: i2c_transfer() %d", ret);
	if (ret < 0)
		return ret;
	if (ret != i)
		return -EIO;
...
static int edt_ft5x06_factory_mode(struct edt_ft5x06_ts_data *tsdata)
{
	...
	if (retries == 0) {
		printk(KERN_INFO  "not in factory mode after %dms.\n",
...
static int edt_ft5x06_work_mode(struct edt_ft5x06_ts_data *tsdata)
{
	...
	if (retries == 0) {
		printk(KERN_INFO  "not in work mode after %dms.\n",
			EDT_SWITCH_MODE_RETRIES * EDT_SWITCH_MODE_DELAY);
		tsdata->factory_mode = true;
		return -EIO;
	}
					 ...
					 int reset_pin)
{
	...
	if (gpio_is_valid(reset_pin)) {
        ...
        printk(KERN_INFO "EDT: reseting the chip\n");
		mdelay(50);
					    ...
					    char *fw_version)
{
    ...
    printk(KERN_INFO "EDT: got %d", error);
					 ...
					 const struct i2c_device_id *id)
{
    ...
    printk(KERN_INFO "starting EDT\n");
	printk(KERN_INFO "probing for EDT FT5x06 I2C\n");
	...
	if (gpio_is_valid(pdata->irq_pin)) {
        printk(KERN_INFO "EDT Got an irq_pin %d\n", pdata->irq_pin);
	...
	if (!tsdata || !input) {
		printk(KERN_INFO  "failed to allocate driver data.\n");
		error = -ENOMEM;
		goto err_free_mem;
	}
	...
	if (error) {
		printk(KERN_INFO  "touchscreen probe failed\n");
		goto err_free_mem;
	}
	...
	if (error) {
		printk(KERN_INFO  "Unable to init MT slots.\n");
		goto err_free_mem;
	}
	...
	error = request_threaded_irq(client->irq, NULL, edt_ft5x06_ts_isr,
				     ...
				     client->name, tsdata);
	if (error) {
		printk(KERN_INFO  "Unable to request touchscreen IRQ.\n");
		goto err_free_mem;
	}
...
/* NOTE:
 * This driver is from newer kernel, so it needed some modifications to work on
 * this one:
 *
